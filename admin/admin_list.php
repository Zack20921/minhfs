<?php
require 'doimkad.php';
$admin = get_all_admin();
disconnect_db();
?>

<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Change password</title>
    <link rel="stylesheet" href="../tailieu4.css">
    <link rel="icon"type="image/png" href="images/logo.png">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.2.1/css/all.min.css">
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css">
    
</head>
<style>
</style>
<body>
        <!-----Menu---->
      <section class="s1" id="s1">
          <a href="../admin/admin.php" class="logo">Zack</a>
          <input type="radio" name="slider" id="menu-btn">
          <input type="radio" name="slider" id="close-btn">
          
          <ul style="margin:0;">
              <li><a href="../admin/admin.php" >Back</a></li>
          </ul>
          <label for="menu-btn" class="btn menu-btn"><i class="fas fa-bars"></i></label>
      </section>
        <!--------section5--------> 
      <section class="s2" id="s2">
        <div class="container">
            <h1>Thông tin tài khoản mật khẩu admin</h1><br>
        </div>

        <table  width="100%" border="1">
          <tr style="border-top: 0px solid #fff; border-bottom: 2px solid rgb(65, 64, 65)">
          <td><b>Tên tài khoản</b></td>
          <td><b>Mật khẩu</b></td>
          <td><b>Vai trò</b></td>
          <td><b>Sửa</b></td>
          </tr>

          <?php foreach ($admin as $item) { ?>
            <tr id="tr1" >    
              <td class="dt"><?php echo $item['Ten_tk']; ?></td>
              <td class="dt"><?php echo $item['Mk']; ?></td>
              <td class="dt"><?php echo $item['Vai_tro']; ?></td>
              <td id="tr1">
                  <form method="post" action="admin_delete.php">
                      <a style="margin-right:40px;margin-left:5px " onclick="window.location ='admin_edit.php?id=<?php echo $item['ID_user']; ?>'" 
                      type="button" value="Sửa"><i class="fa-regular fa-pen-to-square"></i></a>
                  </form>
              </td>
            </tr>    
            <?php } ?>          
        </table>                  
</section>
</body>
</html>

