...
$ID_kh = isset($_GET['id']) ? (int)$_GET['id'] : '';
if ($ID_kh){
    $data = get_khoahoc($ID_kh);
    // $role = getRole_id();
    // $pd = getPB_id();
}
...
if (!empty($_POST['edit_khoahoc']))
{
    ...
    $data['ID_kh']          = isset($_POST['id']) ? $_POST['id'] : '';
    ...
    if (!$errors){
        edit_khoahoc($data['ID_kh'], $data['Ten_kh'], $data['Ngay_bat_dau'], $data['Ngay_ket_thuc'], $data['ID_gv'], $data['Hoc_phi'], $data['Do_kho'], $data['Chi_tiet'], $data['Phan_loai']);
...
<html>
    <head>
        <title>Sửa thông tin khóa học</title>
        <meta charset="UTF-8">
    ...
    <section class="s2" id="s2">
        ...
        <form method="post" action="khoahoc_edit.php?id=<?php echo $data['ID_kh']; ?>">
        <h1>Update course info</h1>
        <a href="khoahoc_list.php">Back</a> 
            ...
            <input type="hidden" name="id" value="<?php echo $data['ID_kh']; ?>"/>
            <input id="ip2" type="submit" name="edit_khoahoc" value="Save"/>
        </form>
    </section>
    </body>
</html>
